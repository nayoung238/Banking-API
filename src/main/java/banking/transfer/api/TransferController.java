...
public class TransferController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "송금 성공", content = @Content(schema = @Schema(implementation = TransferDetailResponse.class))),
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "계좌의 모든 거래 조회 완료", content = @Content(schema = @Schema(implementation = TransferSummaryResponse.class))),
        @ApiResponse(responseCode = "404", description = "존재하지 않는 계좌", content = @Content(schema = @Schema(implementation = String.class))),
        @ApiResponse(responseCode = "403", description = "해당 계좌에 대한 접근 권한 없음", content = @Content(schema = @Schema(implementation = String.class)))
    })
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "거래 상세 내역 조회 완료", content = @Content(schema = @Schema(implementation = TransferSummaryResponse.class))),
    ...
    @GetMapping
    public ResponseEntity<?> findTransferDetails(@RequestBody @Valid TransferDetailsRequest request,
                                                 @AuthenticationPrincipal UserPrincipal userPrincipal) {
        return ResponseEntity
                .status(HttpStatus.OK)
